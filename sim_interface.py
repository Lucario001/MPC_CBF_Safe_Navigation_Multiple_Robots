...
class Pioneer:
    ...
    def robot_at_goal(self):
        ...
        self.localize_robot()
        return True if np.sqrt((self.current_state[0] - self.goal_state[0]) ** 2 + (self.current_state[1] - self.goal_state[1]) ** 2) <= robot_params.goal_threshold else False
    ...
    """
    Created on Mon Jul 15 16:57:54 2024
    
    @author: kanni
    """
    
    def run_MPC_Controller(self, V, W, T):
        if not self.robot_at_goal():
            self.setvel_pioneer(V, W)
            time.sleep(T)
        else:
